...
export default function DiagnosticoPage() {
  const [status, setStatus] = useState<DiagnosticoStatus | null>(null)
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch('http://localhost:3001/api/v1/movements', {
        ...
        .then(r => r.json())
        .then(data => {
          setStatus((s) => s ? { ...s, apiResponse: data, apiStatus: 'OK' } : s)
        })
        .catch(err => {
          setStatus((s) => s ? { ...s, apiError: err.message, apiStatus: 'ERRO' } : s)
        })
    }
  }, [])
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {!status ? (
        ...
        <div className="space-y-4">
          ...
          {status.temToken && status.apiResponse && status.apiResponse.data && status.apiResponse.data.length > 0 && (
